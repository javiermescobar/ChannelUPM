...
class NewsRepository {
    ...
    suspend fun getNews(userId: Int): Response<List<New>> {
    ...
    suspend fun addNew(userId: Int, title: String, description: String, categoryId: Int): Response<Int> {
        return RetrofitInstance.newsApi.addNew("https://zndva5soirzksyn3hs62ls4s5y0vqpvx.lambda-url.eu-west-2.on.aws/",
        userId = userId,
        title = title,
    ...
    suspend fun editNew(newId: Int, title: String, description: String, categoryId: Int): Response<Int> {
        return RetrofitInstance.newsApi.editNew("https://gkmkxysvl4idar6n57qgs7xkse0bokvs.lambda-url.eu-west-2.on.aws/",
        newId = newId,
        title = title,
        description = description,
        categoryId = categoryId)
